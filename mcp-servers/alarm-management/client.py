...
logger = logging.getLogger("alarm_mcp.client")
...
class AlarmManagementClient:
    """HTTP client for the Alarm Management API simulator — the only place
    ...
    def __init__(self) -> None:
        ...
        self._token = settings.alarm_api_token
        self._max_retries = settings.max_retries
        self._client = httpx.AsyncClient(
            base_url=settings.alarm_api_base_url, timeout=settings.request_timeout_seconds
        )
    ...
    async def request(
        ...
        *,
        ...
        """Call the Alarm API with retry + structured error mapping.

        Args:
            method: HTTP method, e.g. "GET" or "POST".
            path: path relative to the base URL, e.g. "/assets/search".
            params: query parameters.
            json_body: JSON body for POST endpoints.
        ...
        raise UpstreamError(
            ...
            detail=f"Alarm API unreachable after {self._max_retries} attempts: {last_exc}",
            status_code=None,
            trace_id=trace_id,
        )
